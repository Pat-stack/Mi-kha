...
static void init_pen_device(XDeviceInfo *info, struct x11_pen_device *pen, bool eraser) {
	...
	if (device == NULL) {
		// XOpenDevice can fail (returns NULL) for devices that match by name but are not
		// currently openable, e.g. a tablet stylus while the pen is out of proximity.
		// Bail out instead of dereferencing a NULL XDevice below.
		return;
	}
